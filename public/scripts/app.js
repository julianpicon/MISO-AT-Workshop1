...
(function () {
    ...
    app.getSchedule = function (key, label) {
          ...
          caches.match(url).then(function(response) {
            if (response) {
              response.json().then(function updateFromCache(json) {
                var results = json.result.schedules;
                results.key = key;
                results.label = label;
        ...
        request.onreadystatechange = function () {
            window.APIRequestTime = performance.now();
            console.log("vwindow.APIRequestTime: " + window.APIRequestTime);
   ...
   localforage.setItem('dev', 'julian_picon');

    // Save list of station to localStorage.
  app.saveSelectedTimeTables = function() {
    ...
    localforage.setItem('dev', 'Julian Picon');
